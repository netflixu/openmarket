...
- 싸우지 말고 다 함께 으쌰으쌰 하여 서로 도와주며 행복하게 프로젝트 마무리하기.
...
- **백엔드 의존성 최소화**  
  정적 호스팅 환경(GitHub Pages)에서도 동작할 수 있도록, SPA 구조로 구축하여
...
| main | '/#productList' | productList    | templates/productList.html | 상품 목록 화면 |
| main | '/#product'     | product        | templates/product.html     | 상품 상세 화면 |
...
3. 빌드 및 watch 명령어 실행 (빌드 시 output.css 생성, '--watch' 모드는 실시간 빌드 모드)
...
구매자 탭에서 가입 시 구매자, 판매자 탭에서 가입 시 판매자로 등록
...
동일 상품 중복 추가 방지 - 도전 과제

### GNB

비로그인/구매 회원: 검색 + 장바구니

비로그인 상태에서 장바구니/바로 구매 클릭 → 로그인 모달
...
#### SPA(Single Page Application)는 전체 페이지 새로고침 없이 필요한 부분만 동적으로 교체해 렌더링하는 방식이다.

#### 구현 방식

- 해시 기반 라우팅 – /#login, /#product?id=2 등 hash 감지
- 페이지 템플릿 로딩 – /templates/\*.html fetch 후 #app에 삽입
- 동적 스크립트 로딩 – loadScript()로 중복 방지 + init 실행 보장
- 공통 컴포넌트 관리 – Header/Footer를 별도 fetch 삽입
- 에러 처리 – 잘못된 라우트 시 404.html 로드
- 메타 태그 변경 – 페이지별 title/description 업데이트

#### 왜 SPA인가?

- 빠른 화면 전환, 서버 의존 최소화
- 템플릿/스크립트 분리로 협업·유지보수 용이
- 프레임워크 없이 SPA 원리 직접 구현해보면서 기술학습 효과

### 7.2 정규화

- 목적: 프로젝트 내에서 코드 스타일과 구조를 일관성 있게 맞추는 작업을 진행
- 방법
  - 네이밍 규칙, 파일구조와 import 정리, 스타일 동일 등
  - Prettier + Tailwind CSS 플러그인을 사용하여 class 순서 및 코드 포맷 자동화
- 효과
  - 협업 시 가독성 향상, 유지보수 용이, 불필요한 중복 코드 제거.
  - 데이터 구조가 달라도 동일한 방식으로 화면 렌더링 가능하며 이후 정렬(getIdComparator)이나 필터링 로직에서 안정적으로 활용 가능

### 7.3 CLI를 이용한 Tailwind CSS 사용
...
- 판매자가 등록한 상품을 카드 형태로 표시 (상품명·판매자명·가격·이미지)
- /products API 연동 → 데이터 반복 렌더링 후 DOM 삽입
- 상품 클릭 시 → /#product?id={상품ID}로 이동

### 7.7 상품상세(:id)

- URL 파라미터(id) 기반 API 호출 → 데이터 바인딩
- 특정 상품의 상세 정보 제공 (이미지·이름·가격·재고·수량 조절)
- 수량 조절·재고 검증, 권한에 따른 버튼 제어, 총 가격 실시간 계산

### 7.8 GNB 로그인에 따른 상태 관리
...
- 로그인이 필요한 기능(예: 마이페이지, 장바구니 등)에 비로그인 상태로 접근 시, 로그인 유도 모달을 띄워 사용자에게 안내한다.
- 모달은 ESC 키나 외부 클릭, 닫기 버튼 등 다양한 방법으로 닫을 수 있도록 구현한다.
- 모달의 접근성(ARIA 속성, 포커스 트랩 등)을 고려하여 키보드 사용자도 자연스럽게 사용할 수 있도록 구현한다.
...
- innerHTML 교체 시 JS 실행 안 됨 → 각 페이지 로직이 동작하지 않음
- 동일 페이지 재진입 시 script 중복 삽입 → 전역 충돌 발생
- 비동기 타이밍 문제 → DOM 준비 전 init 함수 실행 오류
- 에러 처리 없음 → 잘못된 URL 접근 시 빈 화면 출력

#### 원인 분석

- HTML만 교체하면 `<script>` 실행이 보장되지 않음
- script 태그를 무분별하게 삽입해 전역 스코프 충돌 발생
- fetch 비동기 특성 때문에 초기화 순서 보장 실패
- 예외 상황(404 등)에 대한 폴백 로직 미구현

#### 해결

- 동적 스크립트 로더: id 부여 후 중복 로드 방지
- script.onload 활용: 로딩 완료 후 init 함수 실행 보장
- 404 페이지 폴백: 잘못된 경로 시 404.html 로드
- init 함수 패턴 도입: 페이지별 전역 init 함수 호출 체계 확립
- UX 개선: 페이지 이동 시 스크롤 최상단 이동

### 8.2 JavaScript가 로드되지 않는 문제
...
index.html 파일 하나에서 JS를 이용하여 template 파일과 script를 불러와 동적으로 적용하는 방식으로 구현이 되다 보니 meta 태그를 직접 처리해 줘야 하는 상황이 생겼다.

#### 해결

router.js에서 url의 정보를 받아 페이지를 구별한 후 meta 태그를 각 페이지에 맞는 정보로 업데이트하는 방식으로 구현하였다.
이때 routeArray 변수를 만들어 각 라우터에 적용할 값을 미리 정의해두었다. 동적으로 내용이 변경되는 상품 상세 리스트의 경우, 해당 페이지를 로드할 때 meta 태그를 업데이트하도록 구현하였다.

### 8.4 회원가입 페이지 실시간 유효성 검사
...
2. 허지은
   ...
   어떤 방식으로 프로젝트의 구조를 잡고 일정을 짜며 어떤 방식으로 코드를 작성할지 상의하고 업무를 분담하는 모든 과정이 생소하지만 재미있었습니다.
   ...
   개인 프로젝트를 진행할 때도 느꼈지만, 꼭 다 했다 싶으면 오류들이 하나씩 삐져나오는 걸 보면서 끝날 때까지 끝난 게 아니구나 했습니다. 부족한 저와 같이 열심히 열 내면서 작업한 팀원분들께 감사 인사드립니다!!

3. 김규호

   프로젝트 수행이라는 하나의 목표로 팀이 되어 진행한 과정에서 협업에 대한 여러 가지 상황을 경험했습니다. GitHub과 같은 도구를 사용하여 협업하는 과정이 예상보다 흥미진진하고 스릴 있는 경험이었습니다. 개인 프로젝트와 마찬가지로, 최초에 수립하는 계획이나 기준이 명확할수록 개발 과정이 수월해진다고 느꼈습니다. 함께 일하기 위한 기준이 명확할수록 협의 과정이 원활해진다는 것을 느꼈습니다.
   같은 팀이 되어, 함께 프로젝트를 진행하신 팀원들의 노고에 감사드립니다.

4. 안민

   작다면 작고, 크다면 큰 프로젝트가 이렇게 종료되었네요. 이번 프로젝트를 하면서 참 많은걸 배워가는거 같네요. 항상 많은 도움을 준 팀원분들께 감사드리고, 앞으로도 화이팅입니다!
